...
```
docker run --name OpenMediaVault -d -ti -h openmediavault.example.test -p 8100:80 -v /path/to/data:/data:Z -P bahricanli/openmediavault
```
...
```
docker run --name OpenMediaVault -d -ti -h openmediavault.example.test -v /path/to/data:/data:Z -P bahricanli/openmediavault
```
